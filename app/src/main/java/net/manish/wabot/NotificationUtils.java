...
public class NotificationUtils
{
    ...
    public static NotificationCompat.Action getQuickReplyAction(Notification notification)
    {
        for (int i = 0; i < NotificationCompat.getActionCount(notification); i++)
        {
            ...
            if (Objects.requireNonNull(action).getRemoteInputs() != null)
            {
                for (RemoteInput resultKey : Objects.requireNonNull(action.getRemoteInputs()))
                {
    ...
    {
        for (NotificationCompat.Action next : new NotificationCompat.WearableExtender(notification).getActions())
        {
